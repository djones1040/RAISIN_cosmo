...
def plot_cmd(genversion,cid_list,nml,isdist,private):
	...
	if nml is not None:
		if cid_list is not None:
			...
			cmd="snlc_fit.exe "+nml+" VERSION_PHOTOMETRY "+genversion+" MXEVT_PROCESS 40 SNTABLE_LIST "+\
				"'FITRES(text:key) SNANA(text:key) LCPLOT(text:key) SPECPLOT(text:key)' TEXTFILE_PREFIX OUT_TEMP_"+rand+" > OUT_TEMP_"+rand+".LOG"
	else:
...
def main():
	...
	if options.CID=="None":
		if options.dist or options.fitres_filename is not None:
			print("No CID given, assuming all for distributions, then first 40 for LC/SPEC plotting...")
		else:
			print("No CID given, assuming first 40...")
		options.CID=None
		all_cid=True
	elif '-' in options.CID:
