...
class txtobj:
	...
	def writefitres(self,filename,clobber=False):

		#import pdb; pdb.set_trace()
		varnames = [v for v in self.__dict__.keys() if isinstance(self.__dict__[v],np.ndarray) and v != 'VARNAMES:']

		if os.path.exists(filename) and not clobber:
			raise RuntimeError(f'file {filename} exists!')

		with open(filename,'w') as fout:
			header = f"""# FITRES output from txtobj.py
...
#"""
			...
			for j in range(len(self.__dict__[varnames[0]])):
				outvars = ' '.join([str(self.__dict__[v][j]) for v in varnames])
				outline = f"SN: {outvars}"
