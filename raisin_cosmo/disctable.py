...
def main():
    ...
    for files in [glob.glob('data/photometry/PS1_RAISIN/*.snana.dat'),
        ...
        for f in np.array(files)[idx]:
            ...
            if sn.SNID.startswith('PSc'):
                print(f"PS1-{sn.SNID[-6:]}&{ra_str}&{dec_str}&{float(sn.REDSHIFT_FINAL.split()[0]):.3f}&{first_mjd:.0f}\\\\")
            else:
...
def zhel():
    ...
    for files in [glob.glob('data/photometry/PS1_RAISIN/*.snana.dat'),
        ...
        for f in np.array(files)[idx]:
            ...
            first_mjd = np.min(sn.MJD[sn.FLUXCAL/sn.FLUXCALERR > 5])
            if sn.SNID.startswith('PSc'):
                print(f"PS1-{sn.SNID[-6:]}&{ra_str}&{dec_str}&{float(sn.REDSHIFT_HELIO.split()[0]):.3f}&{float(sn.REDSHIFT_FINAL.split()[0]):.3f}&{first_mjd:.0f}\\\\")
            else:
                print(f"{sn.SNID}&{ra_str}&{dec_str}&{float(sn.REDSHIFT_HELIO.split()[0]):.3f}&{float(sn.REDSHIFT_FINAL.split()[0]):.3f}&{first_mjd:.0f}\\\\")
