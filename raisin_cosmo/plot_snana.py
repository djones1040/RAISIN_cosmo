...
def plot_lc(cid,base_name,noGrid,plotter_choice,tmin,tmax,filter_list,plot_all,fitres=None):
	if tmin is None:
		tmin=-np.inf
	if tmax is None:
		tmax=np.inf
...
def plot_cmd(genversion,cid_list,nml,isdist,private):
	...
	if nml is not None:
		if cid_list is not None:
			...
			cmd="snlc_fit.exe "+nml+" VERSION_PHOTOMETRY "+genversion+" MXEVT_PROCESS 1000 SNTABLE_LIST "+\
				"'FITRES(text:key) SNANA(text:key) LCPLOT(text:key) SPECPLOT(text:key)' TEXTFILE_PREFIX OUT_TEMP_"+rand+" > OUT_TEMP_"+rand+".LOG"
	else:
...
def output_fit_res(fitres,filename):
	with open(os.path.splitext(filename)[0]+'.fitres','w') as f:
		f.write("VARNAMES: CID DLMAG DLMAGERR STRETCH STRETCHERR AV AVERR\n")
		for cid in fitres.keys():
			f.write("SN: %s %f %f %f %f %f %f\n"%(cid,
												fitres[cid]['DLMAG'][0],
												fitres[cid]['DLMAG'][1],
												fitres[cid]['STRETCH'][0],
												fitres[cid]['STRETCH'][1],
												fitres[cid]['AV'][0],
												fitres[cid]['AV'][1]))

def create_dists(fitres,param,joint_type):

	res={p:[] for p in ['DLMAG','STRETCH','AV']}
	reserr={p:[] for p in ['DLMAG','STRETCH','AV']}
	if param is not None:
		res[param] = []
	for cid in fitres.keys():
		for p in ['DLMAG','STRETCH','AV']:
			try:
...
def main():
	...
	parser.add_option("-o","--outdir",help="output directory",action="store",dest="outdir",type='str',default=None)
	...
	if options.CID=="None":
		if options.dist or options.fitres_filename is not None:
			print("No CID given, assuming all for distributions, then first 1000 for LC/SPEC plotting...")
		else:
			print("No CID given, assuming first 10...")
		options.CID=None
		all_cid=True
	elif '-' in options.CID:
	...
	if 'hi': #options.fitres_filename is None:
		...
		filename=options.outdir + '/' + options.version+'.pdf'
		num=0
		...
		with PdfPages(filename) as pdf:
			...
			for cid in options.CID:
				...
				if options.spec:
					...
					figs,fits=plot_lc([cid],options.base_name,options.noGrid,plotter_choice,options.tmin,options.tmax,options.filt_list,options.plot_all,fitres=fitres)
					for f in figs:
						pdf.savefig(f)
				else:
					figs=plot_spec([cid],options.bin_size,options.base_name,options.noGrid)
		...
		if not options.noclean:
			for x in glob.glob(options.base_name+'*'):
				os.remove(x)
			for x in glob.glob(f'OUT_TEMP_*'):
				os.remove(x)

	else:
		print("Creating distributions from FITRES file...")
		filename=options.outdir + '/' + options.version+'.pdf' if options.version is not None else os.path.splitext(options.fitres_filename)[0]+'.pdf'
		num=0
		#if os.path.exists(filename):
		#	filename=os.path.splitext(filename)[0]+'_'+str(num)+'.pdf'
		#while os.path.exists(filename):
