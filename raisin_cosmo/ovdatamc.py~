#!/usr/bin/env python
# D. Jones - 1/7/16
#./ovdatamc.py ps1phot_prob_final.fitres ps1sim_empirical.new.fitres MURES:SNRMAX1:PKMJDERR:x1vzHD:zHD:x1:c:cvzHD:x1ERRvzHD:cERRvzHD:mBvzHD:mBERRvzHD:FITPROB:cERR --ylog MURES --journal --clobber --nbins 15 --defaultcuts --dataM -19.33 --simM -19.36 --cutwin SNRMAX1 0 150 --cutwin zHD 0.0 0.75 -o emphist.png --cutwin MURES -2 3 --nplots 4 4
...
class ovhist:
    ...
    def main(self,datafile=None,simfile=None,data=None,sim=None,axes=None,
             letters=True,simcolor='r',simlabel='sim',datalabel='data',simls='-',
             showdata=True,showmeanstd=True,defrcpar=True,showlegend=True,
             ylabel=True,ncol=None):
        if defrcpar: plt.rcParams['figure.figsize'] = (18,4)
        ...
        if 'MU' in self.options.histvar or getMU:
            if 'MU' not in data.__dict__.keys():
                if 'DLMAG' not in data.__dict__.keys():
                    ...
                    data.MU,data.MUERR = data.DLMAG[:],data.DLMAGERR[:]
                from astropy.cosmology import Planck13 as cosmo
                if 'MURES' not in data.__dict__.keys():
                    data.MURES = data.MU - cosmo.distmod(data.zHD).value
            if 'MU' not in sim.__dict__.keys():
                if 'DLMAG' not in data.__dict__.keys():
                    ...
                    sim.MU,sim.MUERR = sim.DLMAG[:],sim.DLMAGERR[:]
                from astropy.cosmology import Planck13 as cosmo
...
#                if histvar == 'zHD': ax.text(0.5,155,'Tuned',fontsize=35,color='r')
            ...
            if not self.options.journal:
                ...
                if ylabel:
                    if letters: ax.set_ylabel(r'N$_{\mathrm{SNe}}$',labelpad=0,fontsize=20)
                    else: axes[0].set_ylabel(r'N$_{\mathrm{SNe}}$',labelpad=0,fontsize=20)
                if 'vzHD' in histvar: 
            ...
            if showdata:
                ax.plot(n_nz[1][:-1]+(n_nz[1][1]-n_nz[1][0])/2.,n_nz[0],'o',color='k',lw=2,label=datalabel)
...
#                    color='b',drawstyle='steps-mid',lw=4,label='Sim. CC SNe',ls='-.')
                ...
                if showlegend:
                    if ncol is not None:
                        ax.legend(loc='center left', #bbox_to_anchor=(0.625, 1.0),
                                  fancybox=True,numpoints=1,ncol=ncol,prop={'size':12})
                    else:
                        ax.legend(loc='center left', #bbox_to_anchor=(0.625, 1.0),
    ...
    def plt2var(self,data,sim,ax,histvar):
        ...
        ax.errorbar((bins[1:]+bins[:-1])/2.,binned_ia,
                    label='sim spec sample',color=simcolor,capsize=0,lw=2)
